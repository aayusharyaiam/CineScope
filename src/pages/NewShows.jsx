...
export default function NewShows() {
  ...
  const renderGrid = (items, ref, cardClass, mediaType, loading, emptyIcon, emptyText, loadingText) => {
    if (loading) {
      return <LoadingState title={loadingText} message="Checking what just arrived..." />;
    }
    if (items.length === 0) {
      return (
        <EmptyState icon={emptyIcon} title={emptyText} message={q ? 'Try a shorter search or a different title.' : 'No fresh titles came back for this shelf yet.'} />
      );
    }
    return (

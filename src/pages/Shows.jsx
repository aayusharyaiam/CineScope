...
export default function Shows() {
  ...
  useEffect(() => {
    ...
    const fetchShows = async () => {
      const data = await tmdbApi.getTrendingShows('week');
      if (!cancelled && data && data.results) setShows(data.results);
      if (!cancelled) setLoadingShows(false);
    };

    const fetchMovies = async () => {
      const data = await tmdbApi.getTrending('week');
      if (!cancelled && data && data.results) setMovies(data.results);
  ...
  const renderGrid = (items, ref, cardClass, mediaType, loading, emptyIcon, emptyText, loadingText) => {
    if (loading) {
      return <LoadingState title={loadingText} message="Building the grid..." />;
    }
    if (items.length === 0) {
      return (
        <EmptyState icon={emptyIcon} title={emptyText} message={q ? 'Try a shorter search or a different title.' : 'The catalog did not return anything for this section.'} />
      );
    }
    return (
  ...
  return (
    <div className="pt-24 pb-20 px-4 md:px-12 max-w-[1440px] mx-auto min-h-screen relative z-10">
      ...
      <section className="mb-20">
        <div className="flex items-end justify-between mb-12">
          <h1 className="font-display text-4xl md:text-5xl font-extrabold text-gray-900 dark:text-white flex items-center gap-4">
            <span className="w-10 h-1.5 bg-gradient-to-r from-brand-coral-pink to-brand-peach-orange rounded-full block"></span>
            Trending TV Shows
          </h1>
        </div>
        {renderGrid(filteredShows, showsGridRef, 'show-card', 'tv', loadingShows, 'live_tv', q ? 'No shows match your search.' : 'No shows available right now.', 'Loading shows...')}
      </section>
      ...
      <section>
        ...
        {renderGrid(filteredMovies, moviesGridRef, 'movie-card', 'movie', loadingMovies, 'movie', q ? 'No movies match your search.' : 'No movies available right now.', 'Loading movies...')}
      </section>
    </div>
  );
}

...
export default function MovieCard({ id, title, year, genre, rating, imageUrl, mediaType = 'movie' }) {
  ...
  return (
    <Link to={linkPath} style={{ width: '200px', minWidth: '200px' }} className="flex-shrink-0 group cursor-pointer snap-start block">
      <div className="relative rounded-xl overflow-hidden aspect-[2/3] mb-2 shadow-lg transition-transform duration-300 group-hover:-translate-y-2 group-hover:shadow-[0_15px_30px_rgba(132,94,194,0.3)]">
        <img 
          src={imageUrl} 
          alt={title} 
